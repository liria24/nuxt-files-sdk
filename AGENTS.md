...
- Single-storage configuration is direct and supports unnamed access. Multiple storages are named and always require a name. Neither construction nor native operation failures switch provider. Environment overrides retain unspecified common options, plugins, and hooks; changing adapters replaces provider config.
- Resolve `$development`, `$production`, `$test`, `$prerender`, and `$env.<name>` before normalization and provider discovery. An active environment without storage is a configuration error; other environments' providers are not generated.
